...
### 2.4 Kernel Argument Metadata (Standard Vortex)

Standard Vortex uses **compile-time coupling** through shared headers - no runtime metadata parsing is required.

#### 2.4.1 Shared Header Pattern

Both host and kernel include the same `common.h` header that defines the kernel argument structure:

**Example**: `vortex/tests/regression/vecadd/common.h`
...
#### 2.4.2 Host Side: Argument Passing

The host creates and populates the `kernel_arg_t` struct, then uploads it to the device:
...
kernel_arg_t kernel_arg;
kernel_arg.num_points = count;
kernel_arg.src0_addr = src0_dev_addr;  // From vx_mem_address()
kernel_arg.src1_addr = src1_dev_addr;
kernel_arg.dst_addr = dst_dev_addr;

// Upload args to device
vx_buffer_h args_buffer;
vx_upload_bytes(device, &kernel_arg, sizeof(kernel_arg_t), &args_buffer);

// Launch kernel - args_buffer address goes to VX_CSR_MSCRATCH
vx_start(device, kernel_buffer, args_buffer);
```

#### 2.4.3 Kernel Side: Argument Access

The kernel reads the argument struct address from the `VX_CSR_MSCRATCH` CSR register:
...
void kernel_body(kernel_arg_t* __UNIFORM__ arg) {
    // Access arguments directly via struct
    auto src0 = reinterpret_cast<float*>(arg->src0_addr);
    auto src1 = reinterpret_cast<float*>(arg->src1_addr);
...
int main() {
    // CSR 0x340 (VX_CSR_MSCRATCH) contains args address
    kernel_arg_t* arg = (kernel_arg_t*)csr_read(VX_CSR_MSCRATCH);
    return vx_spawn_threads(1, &arg->num_points, nullptr, kernel_body, arg);
}
```

#### 2.4.4 Key Points

- **No runtime metadata** - Host and kernel share the same header at compile time
- **Type-safe** - Compiler validates struct layout on both sides
- **Simple** - No JSON parsing or dynamic marshaling
- **Pointer sizes** - Must match target architecture (4 bytes for RV32, 8 bytes for RV64)
...
### 3.4 Kernel Argument Metadata (HIP)

Unlike Standard Vortex which uses shared headers, HIP compilation generates metadata files during the MLIR lowering pass. This enables dynamic argument marshaling at runtime.

#### 3.4.1 Metadata Generation

The `ConvertGPUToVortex` pass emits two files for each kernel:

1. **`<kernel_name>.meta.json`** - Runtime metadata for dynamic argument packing
2. **`<kernel_name>_args.h`** - C header for optional compile-time type-safe usage

**Generation Point**: During `polygeist-opt --convert-gpu-to-vortex` pass

```
kernel.mlir (GPU dialect)
    ↓
[ConvertGPUToVortex pass]
    - Analyzes gpu.func argument types
    - Calculates struct layout (offsets, sizes)
    - Emits metadata files to current directory
    ↓
kernel_vortex.mlir + kernel.meta.json + kernel_args.h
```

#### 3.4.2 JSON Metadata Format

**Example**: `_Z13launch_vecaddPKfS0_Pfji.meta.json`
...
**Key Fields**:
- `is_pointer: true` - Indicates device pointer arguments that need address translation
- `size: 4` - Pointers are 4 bytes on RV32
- `offset` - Byte offset in the packed argument struct

#### 3.4.3 Generated C Header

**Example**: `_Z13launch_vecaddPKfS0_Pfji_args.h`

```c
// Auto-generated kernel argument structure
// Generated by Polygeist ConvertGPUToVortex pass
#ifndef _Z13LAUNCH_VECADDPKFS0_PFJI_ARGS_H
#define _Z13LAUNCH_VECADDPKFS0_PFJI_ARGS_H
...
} _Z13launch_vecaddPKfS0_Pfji_args_t;

#define _Z13LAUNCH_VECADDPKFS0_PFJI_ARGS_SIZE 24

#endif
```

#### 3.4.4 Runtime Metadata Loading

The HIP runtime (`libhip_vortex_runtime`) loads metadata during kernel registration:

```cpp
// hipRegisterKernel loads both .vxbin and .meta.json
hipError_t hipRegisterKernel(const char* kernel_name, const char* kernel_file) {
    // Load kernel binary
    vx_upload_kernel_file(device, kernel_file, &kernel_buffer);

    // Load metadata from <kernel_file_base>.meta.json
    std::string meta_path = kernel_file;
    meta_path = meta_path.substr(0, meta_path.rfind('.')) + ".meta.json";
    loadKernelMetadata(meta_path, info.metadata);

    // Store for use during hipLaunchKernelGGL
...
| MLIR Type | Metadata Type | C Type (RV32) | Size |
|-----------|---------------|---------------|------|
| `memref<?>` | `ptr` | `uint32_t` | 4 bytes |
| `!llvm.ptr` | `ptr` | `uint32_t` | 4 bytes |
| `i32` | `i32` | `int32_t` | 4 bytes |
| `i64` | `i64` | `int64_t` | 8 bytes |
| `f32` | `f32` | `float` | 4 bytes |
| `f64` | `f64` | `double` | 8 bytes |
| `index` | `i32` | `int32_t` | 4 bytes |

#### 3.4.6 Comparison: Standard vs HIP Metadata

| Aspect | Standard Vortex | HIP |
|--------|-----------------|-----|
| **Coupling** | Compile-time (shared header) | Runtime (JSON metadata) |
| **Type Safety** | Compiler-enforced | Optional (generated header) |
| **Flexibility** | Fixed at compile time | Dynamic kernel loading |
| **Overhead** | None | JSON parsing at registration |
| **Files** | `common.h` | `.meta.json` + `_args.h` |
