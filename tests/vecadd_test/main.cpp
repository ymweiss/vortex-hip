// HIP host code for vector addition test
// Adapted from tests/vecadd.cpp for Vortex HIP runtime
...
int main(int argc, char *argv[]) {
  ...
  for (uint32_t i = 0; i < num_points; ++i) {
    h_src0[i] = rand() % 1000;  // Small integers for easy verification
    h_src1[i] = rand() % 1000;
  }
  ...
  // Use hipLaunchKernel with explicit arguments
  // NOTE: Using 4-argument pattern to match metadata generator fallback
